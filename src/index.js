...
const stylesPath = `${componentDir}/styles.ts`;
const typesPath = `${componentDir}/types.ts`;
...
if (!componentName) {
  logError(
    `Sorry, you need to specify a name for your component like this: add-component <name>`
  );
  process.exit(0);
}
...
if (!fs.existsSync(fullPathToParentDir)) {
  logError(
    `Sorry, you need to create a parent "components" directory.\n(add-component is looking for a directory at ${program.dir}).`
  );
  process.exit(0);
}
...
mkDirPromise(componentDir)
  ...
  .then((template) => {
    ...
    .then((template) =>
        // We also need the `types.js` file, which allows easy importing.
    ...
    .then((template) =>
        // We also need the `types.js` file, which allows easy importing.
        writeFilePromise(typesPath, prettify(typesTemplate))
    )
    .then((template) => {
      logItemCompletion('Types file built and saved to disk.');
      return template;
    })
  .then((template) => {
    logConclusion();
